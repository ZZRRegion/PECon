...
VOID ShowMenu()
{
	...
	PRINT_MENU("\timagebase\t- 修改imagebase //imagebase 500000\n");
...
void CmdRelocation(const CHAR* param)
{
		...
		&& pRelocBlock->SizeOfBlock != 0)
	{
		...
		PRINT_INFO("节区\t\t->\t%s\r\n\n", GetSectionName(pSection));
		PRINT_INFO("序号\tTypeOffset\t类型\t\tRVA地址\t\tFOA地址\t\t内容\t\t值偏移\n");
		PRINT_INFO("----------------------------------------------------------------\n");
		for (size_t i = 0; i < entryCount; i++)
		{
			...
			{
				PRINT_INFO("%d\t%04x\t\tHIGHLOW\t\t%08x\t%08x\t%08x\t%08x\n", i, entry, rva, foa, *pValue, *pValue - imagebase);
			}
			else if (type == IMAGE_REL_BASED_ABSOLUTE)
			{
				PRINT_INFO("%d\t%04x\t\tABS\t\t%08x\t%08x\t%08x\t%08x\n", i, entry, rva, foa, *pValue, *pValue - imagebase);
			}
			else
			{
				PRINT_INFO("%d\t0x%04x\t\t%x\t\t0x%08x\t%08x\t%08x\t%08x\n", i, entry, type, rva, foa, *pValue, *pValue - imagebase);
			}
		}
		PRINT_INFO("----------------------------------------------------------------\n");
